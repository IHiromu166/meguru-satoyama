...
export function scoreOf(state: GameState): ScoreBreakdown {
  ...
  const defs = cards.map((card) => definition(card.defId)).filter((card): card is NonNullable<typeof card> => Boolean(card));
...
export function trophicCounts(state: GameState): Record<Trophic, number> {
  ...
  for (const instance of deckCards(state)) { const def = definition(instance.defId); if (def && def.kind !== "invasive" && def.kind !== "control") counts[def.trophic] += 1; }
  return counts;
}
