...
### スクロール位置を保つ (2026-08-25)

`render()` が毎回 `root.innerHTML = ""` で画面全体を作り直していたため、スクロール
している箱ごと別の DOM ノードに入れ替わり、ブラウザがスクロール位置を捨てていた。
手札は画面のかなり下にあるので、1枚プレイするたびに一番上へ戻ってしまう。

**骨組みを作り直さないようにした** (`src/ui/app.ts` の `GameShell` / `buildGameShell`)。
ヘッダ・ログ・`.game-scroll`・各セクション・フェイズ送りボタンはゲーム開始時に1回だけ作り、
以降は `updateGameScreen` が**中身だけ**を入れ替える。要素が生き残っていれば
スクロール位置はブラウザが保つので、位置を退避・復元するコードは持たない。

- 最初は `scrollTop` を退避して復元する形で直したが (2026-08-24)、実機では直っていなかった。
  骨組みごと使い回す形に変えたのが現在の実装。
- フェイズ送りボタンは使い回すので、ハンドラは描画時の `s` ではなく
  モジュール変数の `state` を見る。閉じ込めると1手前の状態で進んでしまう。
- ログの `details` も使い回すため、開閉状態は DOM にそのまま残る。
- タイトル画面・結果画面へ移るときは骨組みを捨てる (`shell = null`)。
...
- **2026-08-25、スクロール位置がプレイ後も動かないことをヘッドレス Edge で確認した。**
  360×640 で `.game-scroll` を 1204 まで下げてカードをプレイ → 1213 (場にチップが増えた分だけ
  ブラウザが追従したもので、見ている位置は変わらない)。1200×800 では `.section-supply` が
  692 のまま不動。どちらも**プレイ前後で同じ DOM ノードのまま**であることを、
  ノードに印を付けて確認した。あわせて 20 ターンの通しプレイ (メイン11回・49枚プレイ →
  結果画面 → もう一度) 、ログの開閉が保たれること、遊び方が開くこと、送りボタンの
  ラベルが正しく推移すること、横のはみ出しゼロ・`console.error` ゼロも確認。
  **実機 (スマートフォン) では未確認。**
